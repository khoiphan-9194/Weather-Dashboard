...
function getWeather(citiName, latitude, longtitude) {
  ...
  fetch(WEATHER_URL)
    ...
    .then(function (data) {
      ...
      $(".weather-detail h3").text(
        citiName.toUpperCase() + " (" + currentDate + ")"
      );
      $(".2").text(
        (((data.main.temp - 273.15) * 9) / 5 + 32).toFixed(0) + "°F"
      );
  ...
  fetch(FORECAST_URL)
    ...
    .then(function (data) {
      ...
      for (let index = 0; index < fiveDay_forecast.length; index++) {
        $(".card" + index + " h3").text(
          foreCastDataList[index].dt_txt.split(" ")[0]
        );
        var url_icon =
          "https://openweathermap.org/img/wn/" +
          foreCastDataList[index].weather[0].icon +
          "@2x.png";
        $(".card" + index + " img").attr("src", url_icon);
        $(".card" + index + " .2").text(
          (((foreCastDataList[index].main.temp - 273.15) * 9) / 5 + 32).toFixed(
            0
          ) + "°F"
        );
...
function getHistory() {
  ...
  $(".histBtn").on("click", function (event) {
    ...
    getWeather(
      parsedObject[$(this).attr("id")].cityName,
      parsedObject[$(this).attr("id")].latitude,
      parsedObject[$(this).attr("id")].longtitude
    );
    getForecast(
      parsedObject[$(this).attr("id")].latitude,
      parsedObject[$(this).attr("id")].longtitude
